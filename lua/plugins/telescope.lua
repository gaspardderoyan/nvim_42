			...
			vim.keymap.set("n", "<leader>fb", function()
				...
				builtin.buffers({
					attach_mappings = function(prompt_bufnr, map)
						-- Map <C-d> in insert mode to close the currently selected buffer
						map("i", "<C-d>", function()
							...
							for _, selection in ipairs(selections) do
								if selection then
									local buf_name =
										vim.fn.fnamemodify(vim.api.nvim_buf_get_name(selection.bufnr), ":t")
									vim.api.nvim_buf_delete(selection.bufnr, { force = true })
									vim.notify(
										"Closed buffer: " .. buf_name,
										vim.log.levels.INFO,
										{ title = "Telescope" }
									)
								end
						...
						-- Keep default telescope behavior (multiselect with <Tab>, open with <CR>, etc)
						-- Return true to keep default mappings intact
						return true
					end,
				})
			end, { desc = "Buffers" })
									...
									if allowed_symbol_kinds[s.kind] then
										...
										if not ignored_symbols[name] then
											...
											local range = s.selectionRange
												or s.range
												or (s.location and s.location.range)
